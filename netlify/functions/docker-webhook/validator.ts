...
export function validateDockerPayload(payload: any): ValidationResult {
  ...
  if (payload.repository?.repo_name && !SUPPORTED_REPOSITORIES.includes(payload.repository.repo_name)) {
    errors.push(`Unsupported repository: ${payload.repository.repo_name}. Supported: ${SUPPORTED_REPOSITORIES.join(', ')}`);
  }
  ...
  if (payload.push_data?.pusher && typeof payload.push_data.pusher !== 'string') {
 ...
 */
export function isValidDockerHubPayload(payload: any): payload is DockerHubPayload {
